package org.jenkinsci.plugins.environmentdashboard;

import org.junit.After;
import org.junit.Before;
import org.junit.Test;

import static org.junit.Assert.*;

// A little change
public class DashboardBuilderTest {

    @Before
    public void setUp() throws Exception {

    }

    @After
    public void tearDown() throws Exception {

    }

    @Test
    public void testGetNameOfEnv() throws Exception {
        //throw new Exception();
    }

    @Test
    public void testGetComponentName() throws Exception {

    }

    @Test
    public void testGetBuildNumber() throws Exception {

    }

    @Test
    public void testGetBuildJob() throws Exception {

    }

    @Test
    public void testSetUp1() throws Exception {

    }

    @Test
    public void testGetDescriptor() throws Exception {

    }

    @Test
    public void testSetUp2() throws Exception {

    }

    @Test
    public void testSetUp3() throws Exception {

    }

    @Test
    public void testDecorateLauncher() throws Exception {

    }

    @Test
    public void testDecorateLogger() throws Exception {

    }

    @Test
    public void testPreCheckout() throws Exception {

    }

    @Test
    public void testGetProjectAction() throws Exception {

    }

    @Test
    public void testGetProjectActions() throws Exception {

    }

    @Test
    public void testMakeBuildVariables() throws Exception {

    }

    @Test
    public void testMakeSensitiveBuildVariables() throws Exception {

    }

    @Test
    public void testAll() throws Exception {

    }

    @Test
    public void testGetDescriptor1() throws Exception {

    }
}